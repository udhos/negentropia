...
func loadModelRadius(model, objURL string) float64 {
	...
	dy := maxY - minY
	dz := maxZ - minZ
	radius := math.Sqrt(float64(dx*dx+dy*dy+dz*dz)) / 2.0
	return radius
}
